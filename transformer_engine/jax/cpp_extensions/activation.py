...
ActivationEnum = {
    ...
    ("oss_silu", "clamped_linear"): NVTE_Activation_Type.OSS_SWIGLU,
}


def _convert_to_activation_function(fn_or_string, act_param: dict = None):
    """Convert a string to an activation function."""
    if fn_or_string == "linear":
        return lambda x: x
    if fn_or_string == "clamped_linear":
        assert act_param is not None and "limit" in act_param
        return lambda x: jnp.clip(x, a_min=-act_param["limit"], a_max=act_param["limit"]) + 1
    ...
    if fn_or_string == "oss_silu":
        return lambda x: _convert_to_activation_function("quick_gelu")(jnp.minimum(x, act_param["limit"]))
...
class ActLuPrimitive(BasePrimitive):
    ...
    impl_static_args = (
        ...
        8,
        9,
    )  # out_dtype, act_enum, act_len, scaling_mode, is_2x, scale_dtype, is_outer, limit
    ...
    def lowering(
        ...
        *,
        ...
        out = ffi.ffi_lowering(ActLuPrimitive.name)(
            ctx, x, scale, act_enum=act_enum, scaling_mode=scaling_mode.value, is_2x=is_2x, limit=limit
        )
        return out
    ...
    def shardy_sharding_rule(
        ...
        del out_dtype, act_enum, act_len, scale_dtype, is_outer, mesh, result_types, limit
...
class BaseDActLuDBiasQuantizePrimitive(BasePrimitive):
    ...
    multiple_results = True
    # out_dtype, scaling_mode, is_2x, scale_dtype, is_dbias, act_enum, act_len, is_outer, limit
    ...
    def infer_sharding_from_operands(
        ...
        del out_dtype, result_infos, act_enum, limit
    ...
    def shardy_sharding_rule(
        ...
        del out_dtype, scale_dtype, act_enum, act_len, is_outer, mesh, result_types, limit
...
def _jax_act_lu(inputs, activation_type, quantizer=None, **act_params) -> Union[NoScaleTensor, ScaledTensor]:
    """
...
def _jax_quantize_dact_dbias(
    ...
    **act_param
    ...
    _, vjp_func = jax.vjp(
        partial(_jax_act_lu, activation_type=activation_type, **act_param), x.astype(jnp.float32)
    )
...
def act_lu(
    ...
    **act_param
    ...
    if not ActLuPrimitive.enabled():
        return _jax_act_lu(x, activation_type, quantizer, **act_param)
    ...
    war_output = try_apply_delayed_scaling_2x_war(
        f=act_lu, x=x, activation_type=activation_type, quantizer=quantizer, **act_param
    )
    ...
    if quantizer is None:
        out, _, _, _, _ = ActLuPrimitive.outer_primitive.bind(
            ...
            is_outer=True,
            limit=np.float32(act_param.get("limit", 7.0)),
        )
...
def quantize_dact_dbias(
    ...
    **act_param
    ...
    if not PrimitiveClass.enabled() or (
        ...
        return _jax_quantize_dact_dbias(dz, x, activation_type, is_dbias, quantizer, **act_param)
    import numpy as np
    if quantizer is None:
    ...
    if should_apply_1x_fused_dbias_war_for_arch_l_100(is_dbias=is_dbias, quantizer=quantizer):
        out = dact_lu(
            dz.astype(jnp.float32), x.astype(jnp.float32), activation_type, quantizer=None, **act_param
        )
    ...
    if is_dbias and is_gated:
        dgated = dact_lu(
            dz.astype(jnp.float32), x.astype(jnp.float32), activation_type=activation_type, **act_param
        )
        out, dbias = _quantize_dbias_impl(

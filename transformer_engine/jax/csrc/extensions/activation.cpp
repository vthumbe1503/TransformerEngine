...
Error_Type ActLuFFI(cudaStream_t stream, Buffer_Type input_buf, Buffer_Type scale_buf,
  ...
  switch (act_type) {
    ...
    case NVTE_Activation_Type::SREGLU:
      ...
      break;
    case NVTE_Activation_Type::OSS_SWIGLU:
      nvte_gptoss_swiglu(input_tensor.data(), output_tensor.data(), limit, 1.702f, stream);
      break;
    default:
...
Error_Type DActLuDBiasQuantizeFFI(cudaStream_t stream, Buffer_Type input_buf,
                                  ...
                                  int64_t act_enum, bool is_2x, bool is_dbias, float limit) {
  auto in_dtype = convert_ffi_datatype_to_te_dtype(input_buf.element_type());
  auto out_dtype = convert_ffi_datatype_to_te_dtype(output_buf->element_type());
  ...
  if (is_dbias) {
    ...
    switch (act_type) {
      ...
      case NVTE_Activation_Type::SREGLU:
        ...
        break;
      case NVTE_Activation_Type::OSS_SWIGLU:
        nvte_gptoss_dswiglu(input_tensor.data(), act_input_tensor.data(), output_tensor.data(), limit, 1.702f, stream);
        break;
      default:
        NVTE_ERROR("Unsupported ActivationEnum");
        break;

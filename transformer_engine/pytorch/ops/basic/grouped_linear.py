...
class GroupedLinear(BasicOperation):
    ...
    def _fuser_backward_grouped_tensor(
        ...
        *,
        ...
        # to figure out total tokens. The upstream op (e.g. an activation
        # with a ``prev_op_grad_output_quantizer``) may hand us a quantized
        # tensor; ``bgrad_group_quantize`` / ``group_quantize`` expect a
        # plain tensor, so dequantize first.
        dy_2d = maybe_dequantize(grad_output, dtype).reshape(-1, self.out_features)

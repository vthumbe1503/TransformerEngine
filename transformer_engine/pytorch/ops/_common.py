...
def fuse_grouped_mlp_ops(
    ...
    *,
    ...
    fused_op_cls : type
        ...
        constructor accepting ``fc1``, activation op (passed via the
        ``activation_kwarg`` keyword) and ``fc2`` keyword args.
    activation_op_types : tuple, optional
        Allowed types for the middle (activation) op. Defaults to
        ``(ScaledSwiGLU, ScaledClampedQGeGLU)``.
    activation_kwarg : str, default = ``"swiglu"``
        Keyword name used when passing the middle op to ``fused_op_cls``.
    ...
    while len(window) == 3:
        ...
        if not (
            ...
            isinstance(window[1], (ScaledSwiGLU, ScaledClampedQGeGLU))

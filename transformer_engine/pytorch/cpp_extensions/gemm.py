...
def get_grouped_gemm_setup_workspace_size(num_tensors: int) -> int:
    ...
    size = 6 * (num_tensors * ptr_bytes) + 6 * (num_tensors * int_bytes)
    alignment = 256
...
def general_grouped_gemm_for_grouped_tensor(
    ...
    *,
    ...
    assert B.num_tensors == num_tensors, "A and B must have the same num_tensors."
    assert out.num_tensors == num_tensors, "Output GroupedTensor has mismatched num_tensors."

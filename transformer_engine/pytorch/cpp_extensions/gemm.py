...
from ..tensor.grouped_tensor import GroupedTensor, GroupedTensorStorage
...
def general_grouped_gemm_for_grouped_tensor(
    A: Union[List[torch.Tensor], GroupedTensorStorage],
    B: GroupedTensorStorage,
    out: Union[List[torch.Tensor], GroupedTensorStorage],
    *,
    C: Optional[Union[List[torch.Tensor], GroupedTensor]] = None,
    layout: str = "TN",
    accumulate: bool = False,
    alpha: Optional[torch.Tensor] = None,
    beta: Optional[torch.Tensor] = None,
):
    """Unified grouped GEMM entry point for grouped/list inputs and outputs."""
    assert layout in ("TN", "NN", "NT"), f"GEMM layout {layout} not supported."
    transa = layout[0] == "T"
    transb = layout[1] == "T"

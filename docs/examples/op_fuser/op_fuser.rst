...
.. code-block:: python
    ...
    # Construct MLP with a residual connection.
    fc1 = te.ops.Sequential(
        te.ops.LayerNorm(4096),
...
Internal extra input channels
"""""""""""""""""""""""""""""

Extra inputs and outputs may optionally be identified by a channel
name. Normally, the caller must provide every extra input to a ``Sequential``. A
channel instead lets an extra output from an earlier operation supply
one or more later extra inputs within the same ``OperationFuser``. To
create this connection, assign the same channel name to the output and
inputs. Internally supplied inputs are omitted from the public
``Sequential`` arguments. A named
input without an earlier matching output remains a public extra input.

Using an internal extra input, the residual block above can be
expressed with one ``Sequential``:
...
Internal extra inputs are also useful for mixture-of-experts blocks.
The following example assumes custom ``Dispatch`` and ``Combine`` basic
operations. ``Dispatch`` has one public extra input containing router
probabilities and three extra outputs: split sizes, token probabilities,
and a routing map. ``Combine`` consumes the routing map.
...
.. code-block:: python
    ...
    # extra outputs are still returned.
    y, m_splits, probs, routing_map = moe(x, router_probs)

Internal extra input channels cannot connect operations in different
``OperationFuser`` instances. In particular, an ordinary PyTorch module
inside a ``Sequential`` splits the fusible operations on either side
into separate fusers. The following connection is therefore not
supported:
...
The following conditions apply to internal extra input channels:

- A producer must appear before all of its consumers. Backward edges
  and cycles are not supported.
- A channel has at most one producer in a fuser, but its output may fan
  out to multiple consumers.
- A named input with no producer in the fuser remains public. Each
  public input requires a separate argument, even when multiple inputs
  have the same channel name.
- Every extra output remains public, whether or not it has consumers in
  the fuser.
- An internal connection is formed only when the producer and
  consumers have matching channel names.
...
- The caller passes extra inputs that are not connected internally,
  whether or not they have channel names. Internally connected input
  slots are omitted from the ``Sequential`` arguments. All extra
  outputs appear in the return value.

Basic operations connected by an internal extra input may still be
replaced by registered ``FusedOperation`` implementations. If a fused
operation contains both the producer and consumer, its
``fuser_forward`` and ``fuser_backward`` implementations are
responsible for routing the tensor and its gradient between those basic
operations.
